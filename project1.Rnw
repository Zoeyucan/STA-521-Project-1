...
\section{5b}
<<>>=
df_out_june = df_out%>%filter(actual_date>='2004-06-01')%>%mutate(actual_date=as.factor(actual_date))

u1 = ggplot(data = df_out_june)+geom_boxplot(aes(actual_date, humidity), size = 0.5) + theme_bw()+ theme(axis.text.x = element_text(angle = 0, hjust = 1)) + ylab(label = "Humidity") + coord_flip()


u2 = ggplot(data = df_out_june) + geom_boxplot(aes(actual_date, humid_temp), size = 0.7)+theme_bw()+ theme(axis.text.x = element_text(angle = 0, hjust = 1)) + coord_flip() + ylab(label = "Temperature")
...
u3 = ggplot(data = df_out_sun) + geom_boxplot(aes(actual_date, hamatop_ppfd), size = 0.7)+theme_bw()+ theme(axis.text.x = element_text(angle = 0, hjust = 1)) + coord_flip() + ylab(label = "Hamatop" )

u4 = ggplot(data = df_out_sun)+geom_boxplot(aes(actual_date, hamabot_ppfd), size = 0.7) + theme_bw()+ theme(axis.text.x = element_text(angle = 0, hjust = 1)) + ylab(label = "Hamabot") + coord_flip()
